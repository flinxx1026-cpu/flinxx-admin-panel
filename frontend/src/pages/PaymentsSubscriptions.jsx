    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-6">
          <h3 className="text-dark-400 text-sm mb-2">Last 30 Days Revenue</h3>
          <p className="text-3xl font-bold text-dark-100">{formatCurrency(stats.totalRevenue || 0)}</p>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-dark-700 border-b border-dark-600">
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-dark-200">Amount (₹ INR)</th>
              ...
              {transactions.length > 0 ? (
                transactions.map((tx) => (
                  <tr key={tx.id} className="border-b border-dark-700 hover:bg-dark-700/30">
                    <td className="px-6 py-4 text-dark-300">{tx.user?.email || tx.user?.username || 'Unknown'}</td>
                    <td className="px-6 py-4 text-dark-300">{tx.plan || 'Standard'}</td>
                    <td className="px-6 py-4 text-dark-300 font-semibold">{formatCurrency(tx.amount)}</td>
